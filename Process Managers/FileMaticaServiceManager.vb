'  ---------------------------------------------------------------------------------
'  ---------------------------------------------------------------------------------
'   Document    :  FileMaticaServiceManager.vb
'   Description :  [type_description_here]
'   Created     :  5/9/2023 9:38:52 AM
...
Public Class FileMaticaServiceManager

#Region "Class Variables"

  Private Shared mobjInstance As FileMaticaServiceManager
...
#Region "Singleton Support"
  ...
  Public Shared ReadOnly Property Instance As FileMaticaServiceManager
    Get
      Try
  ...
  Private Shared Function GetInstance(ByVal lpForceRefresh As Boolean) As FileMaticaServiceManager
    Try
      If lpForceRefresh = True Then
        mobjInstance = New FileMaticaServiceManager()
...
#Region "Public Methods"
  ...
  Public Sub StartHost()
    ...
    Try
      ...
      If Not HostIsRunning() Then
        lobjProcessStartInfo = New ProcessStartInfo("FileMaticaServiceHost.exe")
        lobjProcessStartInfo.UseShellExecute = False

        Dim lobjStartProcessArgs As New StartProcessEventArgs(lobjProcessStartInfo)
        StartFMServiceHostProcess(lobjStartProcessArgs)
      End If
  ...
  Public Function HostIsRunning() As Boolean
    Try
      Dim lobjProcesses As Process() = Process.GetProcessesByName("FileMaticaServiceHost")
      If lobjProcesses.Length > 0 Then
        mobjProcess = lobjProcesses.First
        Return True
      Else
        Return False
      End If
...
#Region "Private Methods"
  ...
  'Private Sub StartServiceHost(lpEventArgs As StartProcessEventArgs)
  '  Try

  '    'Dim lintMultipleJobRunnerInvocationDelaySeconds As Integer =
  '    '      ConnectionSettings.Instance.MultipleJobRunnerInvocationDelaySeconds
  '    StartFMServiceHostProcess(lpEventArgs)
  ...
  Private Sub StartFMServiceHostProcess(lpEventArgs As StartProcessEventArgs)
    Try
      Dim lstrProcessPath As String = Path.Combine(lpEventArgs.ProcessInfo.WorkingDirectory, lpEventArgs.ProcessInfo.FileName)
      'LogSession.LogMessage("Starting JobRunner from '{0}' using configuration file '{1}'.",
      'lstrJobRunnerPath, lpEventArgs.ProcessInfo.Arguments)
      Dim lobjProcess As Diagnostics.Process = Diagnostics.Process.Start(lpEventArgs.ProcessInfo)
      lobjProcess.EnableRaisingEvents = True
